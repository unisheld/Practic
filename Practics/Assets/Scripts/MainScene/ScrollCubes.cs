...
public class ScrollCubes : MonoBehaviour
{
    ...
    void Update()
    {
        if (cubes.transform.position.x > 1f)
        ...
        else if(cubes.transform.position.x < -8.5f)
            cubes.transform.position = Vector3.MoveTowards(cubes.transform.position, new Vector3(-8.5f, cubes.transform.position.y, cubes.transform.position.z), Time.deltaTime * 10f);
